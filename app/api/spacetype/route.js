...
export async function GET(request) {
  try {
    ...
    const url = `${backendUrl}/api/v1/SpaceType`;
    ...
    if (!data || Object.keys(data).length === 0) {
      ...
      return NextResponse.json({
        meta: { status: 'Success', messages: [{ text: 'No space types found', type: 'Info' }] },
        data: []
      });
    }
    ...
    console.error('SpaceType API error:', error);
...
export async function POST(request) {
  try {
    ...
    const res = await fetch(`${backendUrl}/api/v1/SpaceType`, {
      method: 'POST',
      headers: {
        Authorization: authHeader,
        'Content-Type': 'application/json',
    ...
    const data = await res.json();
    console.log("Create space type response:", data);
    ...
    console.error('Create SpaceType API error:', error);
    return NextResponse.json(
